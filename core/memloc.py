...
from core.masks import find_mask
...
def apply_mask_to_range(memloc, size, offset):
    ...
    range_pos = add_op(range_pos, sub_op(
        range_len, add_op(size_bytes, offset_bytes)))
    # sub_op(range_len, add_op(offset_bytes, size_bytes))
    range_len = size_bytes
...
def split_or(value):
    ...
    for row in terms:
        ...
        if opcode(row) == "mask_shl" and all_concrete(row):
            row = apply_mask(row[4] if row[4] <
                             256 else 256, row[1], row[2], row[3])
            ...
            if f_off == 0 and s_off == (
                ...
                (
                    ...
                    (
                        ...
                        3,
                        ("add", 32, ("mul", -1,
                         ("mask_shl", 5, 0, 0, f_size[4]))),
                    ),
                ),
            ):
                assert match(
                    s_size, ("mask_shl", Any, Any, Any,
                             ("add", 32, ("mul", -1, ...)))
                )
                return ret_rows
    ...
    try:
        # sort by offsets, descending
        ret_rows.sort(key=lambda row: cmp_to_key(row[1]))
    except Exception:
...
def memloc_overwrite(memloc, split):
    ...
    left_ge_zero, right_ge_zero = safe_ge_zero(
        left_len), safe_ge_zero(right_len)
...
assert splits_mem(("range", 4, 32), ("range", 65, 32), "a") == [
    (("range", 4, 32), "a")]
